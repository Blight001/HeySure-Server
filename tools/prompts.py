...
SYSTEM_PROMPT_USAGE = {
    ...
    "prompt_ai_message_inquiry": "message_type=\"inquiry\" 时注入。模板可提示对方用 message.send+to(message_type=\"reply\") 答复。",
    ...
    "prompt_user_message_notice": "message.send+to 发给用户成功后返回的用户消息送达提示。",
    ...
    "task_plan_flow_prompt": "任务运行时注入的可选分阶段流程说明（复杂任务可用 todo.manage 创建→edit 推进→末阶段自动收尾；简单任务可直接执行）。",
}
