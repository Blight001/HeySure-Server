...
SYSTEM_PROMPT_USAGE = {
    ...
    "prompt_ai_message_inquiry": "message_type=\"inquiry\" 时注入。模板可提示对方用 message.send+to+ai(message_type=\"reply\") 答复。",
    ...
    "prompt_user_message_notice": "message.send+to+user 成功后返回的用户消息送达提示。",
    ...
    "task_plan_flow_prompt": "任务运行时注入的分阶段流程说明（todo.manage 创建→edit 推进→末阶段自动收尾），由系统强制驱动。",
}
