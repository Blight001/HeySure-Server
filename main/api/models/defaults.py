...
DEFAULT_COMPRESSION_PROMPT = """你正在为一段即将压缩的长对话制作“可继续工作的详细交接摘要”。摘要会替代较早的原始消息，因此不得只写泛泛结论，也不得遗漏会影响后续执行的细节。

请按以下结构输出中文 Markdown；没有内容的栏目写“无”，不要删除栏目：
## 对话目标
- 用户的最终目标、真实意图、成功标准

## 关键约束与偏好
- 用户明确要求、禁止事项、环境限制、格式/时间/权限要求

## 已确认事实与重要数据
- 人名、路径、地址、ID、参数、版本、时间、错误信息、关键原文等需要精确保留的信息

## 已完成工作
- 按先后顺序列出采取的动作、修改内容、验证方式及结果

## 关键决策与原因
- 已做出的选择、取舍、被否决的方案及原因

## 当前状态
- 正在进行到哪里，哪些结果已确认，哪些仍是假设或待验证

## 未完成事项与下一步
- 待办、依赖关系、建议执行顺序，以及每项完成标志

## 风险、阻塞与注意事项
- 已知故障、失败尝试、潜在副作用、不可重复踩坑点

要求：
1. 具体、忠实、可操作，优先保留决定后续行为的信息；不要把“做过”写成“已成功”，除非历史中已有验证结果。
2. 区分用户请求、AI 建议、工具实际结果和未验证推断。
3. 保留重要代码符号、命令、文件路径、工具名、消息 ID 和数值，不得擅自改写。
4. 合并重复信息，但保留需求变化、纠正过程和最终采用的结论。
5. 只输出摘要正文，不要添加摘要之外的解释或前后缀。
...
DEFAULT_MCP_NAMESPACE_HINTS = """{"mcp":"MCP 自省入口。使用 mcp.describe+tool 发现工具并读取参数 schema。","member":"AI 数字成员管理。member.manage 查询、创建、编辑成员及其任务与设备绑定，需要绑定图书馆；成员删除只能由人在控制台确认。","todo":"统一计划管理。todo.manage(action=create/get/edit/delete) 创建、查看、推进或删除计划；edit 更新当前阶段状态，最后阶段更新后系统自动收尾。","workspace":"工作区。workspace.run+command 执行命令和文件操作；workspace.file+manage 将文件注册为可安全发送的 file_ref；联网查询用 workspace.search。","device_mcp":"设备清单、设备 MCP 使用范围与动态工具管理（图书馆工具，需绑定图书馆）。","conversation":"会话管理。","knowledge":"knowledge.search 检索知识；knowledge.manage 管理图书馆内容（需绑定图书馆）。","ai":"AI 间通信。","user":"用户通知。","web":"联网搜索。","memory":"长期记忆。","project":"项目管理。"}"""
...
- Do not assume tool arguments. The [动态 MCP 说明] section in the system prompt lists the callable tools. Call tools whose schema is already exposed directly; use mcp.describe+tool (tool / tools / query) only when the target schema is unavailable or genuinely unclear.
