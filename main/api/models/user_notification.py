...
class UserNotification(SQLModel, table=True):
    ...
    push_status: str = Field(default="not_required", index=True)
    push_attempts: int = Field(default=0)
    push_next_attempt_at: float = Field(default=0.0, index=True)
    push_lease_owner: str = Field(default="")
    push_lease_expires_at: float = Field(default=0.0, index=True)
    push_last_error_code: str = Field(default="")
    push_delivered_at: Optional[float] = Field(default=None, index=True)
