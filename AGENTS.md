...
server/
  main/                    ← 运行时核心（4 个进程 + 1 个共享层）
    api/                   ← 共享库（模型 / DB / 认证 / 服务 / 配置）
      ...
      services/            ← 业务逻辑，按域分 7 个子包（见下）
        ...
        chat/              ← 聊天（persistence/media/compress/inject/mcp_session_context）
        access/            ← 访问与治理（access_guards/auth_settings/governance/ice_settings）
      ...
      chat_runtime/        ← 聊天编排（调度 chat_scheduler / 流式 chat_stream / MCP 解析 / run_state）
      ...
      database.py          ← DB 引擎（SQLModel + asyncpg/psycopg）
    ...
    connector_runtime/     ← 进程③ 连接器 (3002)
      ...
      dispatch/            ← 端侧消息分发（device_dispatch 任务队列 + 远程连接：
  ...
  tools/                   ← 工具箱内置设备（server 固定 MCP 工具集，约 11 个模块）
  other/
    migrations/            ← Alembic 迁移版本（versions/ 下约 34 个）
    scripts/               ← 运维脚本
    tests/                 ← pytest 单元测试（other/tests/ 下约 36 个 test_*.py）
...
| `Chat` / `ChatRun` 等 | `models/chat.py` | 会话与单次推理执行记录 |
...
| `WorkshopBinding` | `models/workshop_binding.py` | 知识工坊绑定 |
| 通信相关 | `models/communication.py` | 通道/通知相关模型 |
...
| `chat/chat_inject.py` | 聊天注入辅助 |
| `chat/mcp_session_context.py` | MCP 会话上下文 |
| **`knowledge/`** | |
...
| `access/ice_settings.py` | WebRTC ICE（STUN/TURN）设置 |
| **`storage/`** | |
...
| `devices.py` + `device_*.py` | `/devices` | 设备注册 / 状态 / 分发 |
| `device_tools.py` | `/devices` 相关工具 | 设备工具管理 |
...
| 任务不执行 | `services/tasks/task_system.py` + `chat_runtime/chat_scheduler.py` | Gateway lifespan 未完成（调度器未启动） |
...
- **Gateway lifespan 有副作用**：启动时加载 MCP 插件、重置 presence、启动调度器；重启 Gateway 会触发这些操作。
